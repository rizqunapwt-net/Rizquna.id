...
document.addEventListener("DOMContentLoaded", function () {
    const vw_book_store_progressBar =
        document.getElementById("vw_book_store_elemento_progress_bar");
    if (!vw_book_store_progressBar) return;
    window.addEventListener("scroll", function () {
        const vw_book_store_scrollTop =
            document.documentElement.scrollTop || document.body.scrollTop;
        const vw_book_store_height =
            document.documentElement.scrollHeight -
            document.documentElement.clientHeight;
        const vw_book_store_scrolled =
            (vw_book_store_scrollTop / vw_book_store_height) * 100;
        vw_book_store_progressBar.style.width =
            vw_book_store_scrolled + "%";
    });
});