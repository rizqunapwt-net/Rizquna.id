...
def deploy():
    try:
        ...
        # Clean old/broken containers
        print("Cleaning old containers...")
        # Add 'cf-tunnel-docker-cloudflared-1' to clean list
        ssh.exec_command(f"cd {REMOTE_PATH} && docker stop rizquna_app rizquna_db rizquna_tunnel cf-tunnel-docker-cloudflared-1 2>/dev/null && docker rm rizquna_app rizquna_db rizquna_tunnel cf-tunnel-docker-cloudflared-1 2>/dev/null")
        time.sleep(2)

        # Run with V2
        print("Starting containers (Fast mode)...")
        stdin, stdout, stderr = ssh.exec_command(f"cd {REMOTE_PATH} && docker compose up -d")
        print(stdout.read().decode())
        print(stderr.read().decode())

        print("Waiting 5s for health checks...")
        time.sleep(5)
